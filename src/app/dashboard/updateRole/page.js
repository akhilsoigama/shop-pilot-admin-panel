...
export default function RoleListPage() {
  ...
  const toggleAllForEntity = (entityKeys) => {
    const all = Object.values(entityKeys);
    const allSelected = all.every(k => form.permissions.includes(k));
    setForm(prev => ({
      ...prev,
      permissions: allSelected
        ? prev.permissions.filter(p => !all.includes(p))
        : [...prev.permissions, ...all.filter(k => !prev.permissions.includes(k))]
    }));
  };

  const updateRole = async () => {
    await axios.put(`/api/admin/role/${editing}`, form, {
      headers: { Authorization: `Bearer ${token}` }
    });
  ...
  return (
    <motion.div className="mx-auto max-w-screen-2xl px-2 pt-5 sm:px-4 md:px-6">
      <Card className="shadow-md">
        <CardContent>
          <Typography variant="h5" className="mb-4">Roles</Typography>
                  ...
                  return (
                    <tr key={role._id} className="bg-white dark:bg-[#171717]">
                      ...
                      <td className="p-3 border break-words">
                        {expanded
                          ? <>
                              ...
                              <button className="text-xs text-blue-500 ml-2" onClick={() => toggleExpand(role._id)}>less</button>
                            </>
                          : <>
                              {firstTwo.join(', ')}
                              {hiddenCnt > 0 && (
                                <button className="text-xs text-blue-500 ml-2" onClick={() => toggleExpand(role._id)}>
                                  +{hiddenCnt} more
                                </button>
                              )}
                            </>
                      ...
                      <td className="p-3 border space-x-2">
                        <Button variant="secondary" onClick={() => startEdit(role)}>Edit</Button>
                        <Button variant="destructive" onClick={() => openDelete(role._id)}>Delete</Button>
                      </td>
                    </tr>
                  );
                })}
                  ...
                  <CardContent className="p-4 space-y-3">
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex space-x-2">
                        ...
                        <Button size="sm" variant="destructive" onClick={() => openDelete(role._id)}>Delete</Button>
                      </div>
                    </div>
                    <div className="text-sm leading-relaxed">
                      {expanded
                        ? <>
                            {role.permissions.join(', ')}
                            <button className="ml-1 text-blue-500" onClick={() => toggleExpand(role._id)}>less</button>
                          </>
                        : <>
                            {firstTwo.join(', ')}
                            {hiddenCnt > 0 && (
          ...
          {editing && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              <Card className="mb-6">
                <CardContent>
                  <Typography variant="h6" className="mb-3">Edit Role</Typography>
                  <Input
                      ...
                      <thead className="bg-gray-200 dark:bg-[#171717]">
                        <tr>
                          ...
                          {['create', 'read', 'update', 'delete'].map(act => (
                            <th key={act} className="p-2 border capitalize">{act}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {permissionMatrix.map(entity => (
                          <tr key={entity.name} className="bg-white dark:bg-[#171717]">
                            ...
                            <td className="p-2 border text-center">
                              <input type="checkbox"
                                     checked={Object.values(entity.keys).every(k => form.permissions.includes(k))}
                                     onChange={() => toggleAllForEntity(entity.keys)}
                                     className="w-4 h-4"/>
                            </td>
                            {['create', 'read', 'update', 'delete'].map(act => (
                              <td key={act} className="p-2 border text-center">
                                <input type="checkbox"
                                       checked={form.permissions.includes(entity.keys[act])}
                                       onChange={() => togglePermission(entity.keys[act])}
                                       className="w-4 h-4"/>
                              </td>
                            ))}
                          </tr>
      ...
      <Dialog open={deleteDialog.open} onOpenChange={closeDelete}>
        <DialogContent className="bg-white text-black dark:bg-[#171717] dark:text-white rounded-md max-w-sm">
          ...
          <div className="text-sm mb-4">Do you really want to delete this role? This action cannot be undone.</div>
